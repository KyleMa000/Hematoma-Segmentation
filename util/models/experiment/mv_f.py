...
class MV_F(nn.Module):
    def __init__(self, level):
        super(MV_F, self).__init__()
...
class M3(nn.Module):
    def __init__(self, input_channels):
        ...
        self.TripleOut = nn.Sequential(
            tltorch.FactorizedConv(in_channels=input_channels, out_channels=32, kernel_size=3, order=2, rank='same'),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            tltorch.FactorizedConv(in_channels=32, out_channels=32, kernel_size=3, order=2, rank='same'),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            tltorch.FactorizedConv(in_channels=32, out_channels=1, kernel_size=3, order=2, rank='same')
        )
